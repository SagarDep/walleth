...
class CreateTransactionActivity : BaseSubActivity() {
    ...
    private fun onCurrentTokenChanged() {
        ...
        amountViewModel.setValue(amountViewModel.currentValue, currentToken)
    ...
    private fun onFabClick(isTrezorTransaction: Boolean) {
        if (to_address.text.isEmpty() || currentToAddress == null) {
        ...
        } else if (currentTokenProvider.currentToken.isETH() && amountViewModel.currentValue ?: ZERO + gas_price_input.asBigInit() * gas_limit_input.asBigInit() > currentBalanceSafely()) {
            ...
            if (currentTokenProvider.currentToken.isETH() && currentERC681?.function == null && amountViewModel.currentValue == ZERO) {
            ...
            } else if (!currentTokenProvider.currentToken.isETH() && amountViewModel.currentValue > currentBalanceSafely()) {
    ...
    private fun startTransaction(isTrezorTransaction: Boolean) {
        val transaction = (if (currentTokenProvider.currentToken.isETH()) createTransactionWithDefaults(
                value = amountViewModel.currentValue,
                to = currentToAddress!!,
                ...
                from = currentAddressProvider.getCurrent(),
                input = createTokenTransferTransactionInput(currentToAddress!!, amountViewModel.currentValue)
